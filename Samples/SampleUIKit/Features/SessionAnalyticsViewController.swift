...
class SessionAnalyticsViewController: UITableViewController {
	...
	private func updateSections() {
		sessionItems = [
			[
				UserStatusMenuItem(title: NSLocalizedString("Are on-demand sessions enabled?", comment: ""), status: areOnDemandSessionsEnabled),
				UserStatusMenuItem(title: NSLocalizedString("Is the session recorded?", comment: ""), status: isRecording)
			],
			[
			...
			[
				ActionableMenuItem(title: NSLocalizedString("Process All Demand Sessions", comment: ""), destination: nil)
			]
		]
	}
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		case let actionableItem as ActionableMenuItem:
			let cell = UITableViewCell.dequeue(from: tableView, at: indexPath, reuseIdentifier: Constants.CellIdentifier.sessionAnalytics)
